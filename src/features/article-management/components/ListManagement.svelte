<script lang="ts">
  ...
  const res =  await fetch(`/api/v1/articles/${selectedId}`, { method: "DELETE" }) 
    if (res.ok) {
    articles = articles.filter(article => article.id.toString() !== selectedId);
      } 
    showModal = false;
    selectedId = null;
  };
...
<div>
  ...
  <div class="relative shadow-md sm:rounded-lg overflow-x-hidden">
    <div
        ...
        {#each articles as article}
          ...
          >
            ...
            >
              ...
              className="w-15 h-15 me-3 object-cover inline rounded-lg"
              alt={article.title} 
              />
